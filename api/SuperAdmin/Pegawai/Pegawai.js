...
router.post('/', validator.validate('PostPegawai'), validator.verify, Upload.checkFiles, Upload.Avatar, (req, res) => {
    ...
    try {
        ...
        const allowed = ['nip', 'nama', 'email', 'no_telp', 'alamat', 'avatar', 'tgl_lahir']
        var filtered = FiltExtras.filter_object(req.body, allowed)

        pModel.getData({nip: req.body.nip})
        .then(x => {
            if (x == "Empty") {
                pModel.insertData(filtered)
                .then(result => {
                    if (result) {
                        if (result == false) {
                            listRes.serverError(res)
                ...
                listRes.clientEntity(res, "NIP sudah tersedia!")
            }
        })
